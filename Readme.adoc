...
* Added new convenience API create pipes between processes with `.pipeTo`
* Fixed issue with leading `..` / `os.up` in path segment created from a `Seq`
* Fixed Windows-specific issues with relative paths with leading (back)slash
* Removed some internal use of deprecated API
* ScalaDoc now maps external references to their online sites
