...
object OpTests extends TestSuite{
  ...
  val tests = Tests {
    val res = pwd/'os/'test/'resources/'testdata
    'ls - assert(
...
//      ls(res/'folder2/'folder2b) == Seq()
    ...
    'lsR{
      ...
      intercept[java.nio.file.NoSuchFileException](ls.rec(pwd/'out/'scratch/'nonexistent))
      assert(
    ...
    'readResource{
      'positive{
        'absolute{
          val contents = read(resource/'test/'os/'folder/"file.txt")
          assert(contents.contains("file contents lols"))

          val cl = getClass.getClassLoader
          val contents2 = read(resource(cl)/'test/'os/'folder/"file.txt")
      ...
      'negative{
        ...
        * - intercept[ResourceNotFoundException]{
          read(resource(classOf[test.os.Testing])/'test/'os/'folder/"file.txt")
        }
        * - intercept[ResourceNotFoundException]{
          read(resource(getClass)/'test/'os/'folder/"file.txt")
        }
        * - intercept[ResourceNotFoundException]{
    ...
    'rm{
      // shouldn't crash
      rm! pwd/'out/'scratch/'nonexistent
    }
    'Mutating{
      val test = pwd/'out/'scratch/'test
      rm! test
      mkdir! test
      'cp{
        val d = test/'copying
