 ...
 * <code>os._ApacheZipEntry</code> instead of
 ...
 *   <code>os._ApacheZipEntry</code> instances.</li>
 *   <li>close is allowed to throw IOException.</li>
 * </ul>
 *
 */
public class _ApacheZipFile implements Closeable {
     ...
     */
    private final Map<String, LinkedList<_ApacheZipEntry>> nameMap =
     ...
     */
    private final _ApacheZipEncoding zipEncoding;
     ...
     */
    public _ApacheZipFile(final File f) throws IOException {
        this(f, null);
    }
     ...
     * @throws IOException if an error occurs while reading the file.
     */
    public _ApacheZipFile(final String name) throws IOException {
     ...
     * @throws IOException if an error occurs while reading the file.
     */
    public _ApacheZipFile(final String name, final String encoding) throws IOException {
     ...
     * @throws IOException if an error occurs while reading the file.
     */
    public _ApacheZipFile(final File f, final String encoding) throws IOException {
     ...
     * @throws IOException if an error occurs while reading the file.
     */
    public _ApacheZipFile(final File f, final String encoding, final boolean useUnicodeExtraFields)
        throws IOException {
        ...
        try {
            final Map<_ApacheZipEntry, NameAndComment> entriesWithoutUTF8Flag =
     ...
     */
    public static void closeQuietly(final _ApacheZipFile zipfile) {
        if (zipfile != null) {
            try {
                zipfile.close();
     ...
     *
     * @return all entries as {@link _ApacheZipEntry} instances
     */
    public Enumeration<_ApacheZipEntry> getEntries() {
     ...
     * @since Ant 1.9.0
     */
    public Enumeration<_ApacheZipEntry> getEntriesInPhysicalOrder() {
     ...
     * @return the _ApacheZipEntry corresponding to the given name - or
     * {@code null} if not present.
     */
    public _ApacheZipEntry getEntry(final String name) {
        final LinkedList<_ApacheZipEntry> entriesOfThatName = nameMap.get(name);
     ...
     *
     * @param name name of the entry.
     * @return the Iterable&lt;_ApacheZipEntry&gt; corresponding to the
     * given name
     * @since 1.9.2
     */
    public Iterable<_ApacheZipEntry> getEntries(final String name) {
        final List<_ApacheZipEntry> entriesOfThatName = nameMap.get(name);
     ...
     * @since 1.9.2
     */
    public Iterable<_ApacheZipEntry> getEntriesInPhysicalOrder(final String name) {
     ...
     * @return boolean
     */
    public boolean canReadEntryData(final _ApacheZipEntry ze) {
        return _ApacheZipUtil.canHandleEntryData(ze);
    }
     ...
     */
    public InputStream getInputStream(final _ApacheZipEntry ze)
        ...
        _ApacheZipUtil.checkRequestedFeatures(ze);
        ...
        switch (ze.getMethod()) {
            case _ApacheZipEntry.STORED:
                return bis;
            case _ApacheZipEntry.DEFLATED:
                bis.addDummy();
    ...
    private static final long CFH_SIG =
        _ApacheZipLong.getValue(_ApacheZipOutputStream.CFH_SIG);

    /**
     * Reads the central directory of the given archive and populates
     * the internal tables with _ApacheZipEntry instances.
     *
     ...
     */
    private Map<_ApacheZipEntry, NameAndComment> populateFromCentralDirectory()
        throws IOException {
        final Map<_ApacheZipEntry, NameAndComment> noUTF8Flag = new HashMap<>();

        positionAtCentralDirectory();

        archive.readFully(WORD_BUF);
        long sig = _ApacheZipLong.getValue(WORD_BUF);
        ...
        while (sig == CFH_SIG) {
            ...
            archive.readFully(WORD_BUF);
            sig = _ApacheZipLong.getValue(WORD_BUF);
        }
        return noUTF8Flag;
    }
     ...
     * _ApacheZipEntry from it and adds it to the global maps.
     *
     ...
     */
    private void
        readCentralDirectoryEntry(final Map<_ApacheZipEntry, NameAndComment> noUTF8Flag)
        throws IOException {
        ...
        final int versionMadeBy = _ApacheZipShort.getValue(CFH_BUF, off);
        off += SHORT;
        ze.setPlatform((versionMadeBy >> BYTE_SHIFT) & NIBLET_MASK);

        off += SHORT; // skip version info

        final _ApacheGeneralPurposeBit gpFlag = _ApacheGeneralPurposeBit.parse(CFH_BUF, off);
        final boolean hasUTF8Flag = gpFlag.usesUTF8ForNames();
        final _ApacheZipEncoding entryEncoding =
            hasUTF8Flag ? _ApacheZipEncodingHelper.UTF8_ZIP_ENCODING : zipEncoding;
        ze.setGeneralPurposeBit(gpFlag);

        off += SHORT;

        ze.setMethod(_ApacheZipShort.getValue(CFH_BUF, off));
        off += SHORT;

        final long time = _ApacheZipUtil.dosToJavaTime(_ApacheZipLong.getValue(CFH_BUF, off));
        ze.setTime(time);
        off += WORD;

        ze.setCrc(_ApacheZipLong.getValue(CFH_BUF, off));
        off += WORD;

        ze.setCompressedSize(_ApacheZipLong.getValue(CFH_BUF, off));
        off += WORD;

        ze.setSize(_ApacheZipLong.getValue(CFH_BUF, off));
        off += WORD;

        final int fileNameLen = _ApacheZipShort.getValue(CFH_BUF, off);
        off += SHORT;

        final int extraLen = _ApacheZipShort.getValue(CFH_BUF, off);
        off += SHORT;

        final int commentLen = _ApacheZipShort.getValue(CFH_BUF, off);
        off += SHORT;

        final int diskStart = _ApacheZipShort.getValue(CFH_BUF, off);
        off += SHORT;

        ze.setInternalAttributes(_ApacheZipShort.getValue(CFH_BUF, off));
        off += SHORT;

        ze.setExternalAttributes(_ApacheZipLong.getValue(CFH_BUF, off));
        off += WORD;
        ...
        // LFH offset,
        offset.headerOffset = _ApacheZipLong.getValue(CFH_BUF, off);
     ...
     * _ApacheExtraFieldUtils forces the field to create local header data
     * even if they are never used - and here a field with only one
     * size would be invalid.</p>
     */
    private void setSizesAndOffsetFromZip64Extra(final _ApacheZipEntry ze,
                                                 final OffsetEntry offset,
                                                 final int diskStart)
        throws IOException {
        final _ApacheZip64ExtendedInformationExtraField z64 =
            (_ApacheZip64ExtendedInformationExtraField)
            ze.getExtraField(_ApacheZip64ExtendedInformationExtraField.HEADER_ID);
        if (z64 != null) {
    ...
    private void positionAtCentralDirectory()
        ...
        if (searchedForZip64EOCD) {
            ...
            archive.readFully(WORD_BUF);
            found = Arrays.equals(_ApacheZipOutputStream.ZIP64_EOCD_LOC_SIG, WORD_BUF);
        }
        if (!found) {
            // not a ZIP64 archive
            if (searchedForZip64EOCD) {
    ...
    private void positionAtCentralDirectory64()
        ...
        archive.seek(_ApacheZipEightByteInteger.getLongValue(DWORD_BUF));
        archive.readFully(WORD_BUF);
        if (!Arrays.equals(WORD_BUF, _ApacheZipOutputStream.ZIP64_EOCD_SIG)) {
    ...
    private void positionAtCentralDirectory32()
        ...
        archive.readFully(WORD_BUF);
        archive.seek(_ApacheZipLong.getValue(WORD_BUF));
    }
    ...
    private void positionAtEndOfCentralDirectoryRecord()
        ...
        final boolean found = tryToLocateSignature(MIN_EOCD_SIZE, MAX_EOCD_SIZE,
                                             _ApacheZipOutputStream.EOCD_SIG);
        if (!found) {
     ...
     * entries.</p>
     */
    private void resolveLocalFileHeaderData(final Map<_ApacheZipEntry, NameAndComment>
                                            entriesWithoutUTF8Flag)
        throws IOException {
        for (_ApacheZipEntry zipEntry : entries) {
            ...
            final int fileNameLen = _ApacheZipShort.getValue(SHORT_BUF);
            archive.readFully(SHORT_BUF);
            final int extraFieldLen = _ApacheZipShort.getValue(SHORT_BUF);
            ...
            if (entriesWithoutUTF8Flag.containsKey(ze)) {
                ...
                _ApacheZipUtil.setNameAndCommentFromExtraFields(ze, nc.name,
                                                         nc.comment);
            }

            final String name = ze.getName();
            LinkedList<_ApacheZipEntry> entriesOfThatName = nameMap.computeIfAbsent(name, k -> new LinkedList<>());
    ...
    private boolean startsWithLocalFileHeader() throws IOException {
        archive.seek(0);
        archive.readFully(WORD_BUF);
        return Arrays.equals(WORD_BUF, _ApacheZipOutputStream.LFH_SIG);
    }
     ...
     * @since Ant 1.9.0
     */
    private final Comparator<_ApacheZipEntry> OFFSET_COMPARATOR = (e1, e2) -> {
        if (e1 == e2) {
            return 0;
        }
    ...
    /**
     * Extends _ApacheZipEntry to store the offset within the archive.
     */
    private static class Entry extends _ApacheZipEntry {
