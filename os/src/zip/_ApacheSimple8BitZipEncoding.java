...
/**
 * This _ApacheZipEncoding implementation implements a simple 8 bit character
 ...
 */
class _ApacheSimple8BitZipEncoding implements _ApacheZipEncoding {
     ...
     */
    public _ApacheSimple8BitZipEncoding(final char[] highChars) {
    ...
    public ByteBuffer encode(final String name) {
        ...
        for (int i = 0; i < name.length(); ++i) {
            ...
            if (out.remaining() < 6) {
                out = _ApacheZipEncodingHelper.growBuffer(out, out.position() + 6);
            }

            if (!this.pushEncodedChar(out, c)) {
                _ApacheZipEncodingHelper.appendSurrogate(out, c);
            }
        }
